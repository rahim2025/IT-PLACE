...
export default function CategoryPage() {
  ...
  const seoTitle = category ? category.seo?.title || `${category.name} | ${SITE_NAME}` : "";
  const seoDescription = category
    ? category.seo?.description || category.description || `Browse our full range of ${category.name} products at ${SITE_NAME}, Saudi Arabia.`
    : "";
  const canonical = category ? category.seo?.canonicalUrl || absoluteUrl(`/categories/${category.slug}`) : "";
    ...
    <section className="bg-background py-10 md:py-16">
      {status === "success" && category && (
        <SeoHead
          ...
          keywords={category.seo?.keywords?.length ? category.seo.keywords : [category.name]}
          canonical={canonical}
          ogImage={category.seo?.ogImage || category.image}
          socialTitle={category.seo?.socialTitle}
          socialDescription={category.seo?.socialDescription}
          jsonLd={[
            buildCollectionPageSchema({ name: category.name, description: seoDescription, url: canonical }),
            buildBreadcrumbSchema([{ name: category.name, url: canonical }]),
          ]}
        />
      )}
      {status === "not-found" && <SeoHead title={`Category Not Found | ${SITE_NAME}`} robots="noindex, follow" />}
        ...
        <div className="mt-6">
          {status === "not-found" ? (
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-surface px-6 py-20 text-center">
              ...
              <h1 className="mt-5 text-lg font-bold text-primary">Category not found</h1>
            ...
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-destructive/30 bg-destructive/5 px-6 py-20 text-center">
              ...
              <h1 className="mt-5 text-lg font-bold text-primary">Couldn't load this category</h1>
              <p className="mt-2 max-w-sm text-sm leading-relaxed text-muted-foreground">
            ...
            <>
              {status === "loading" ? (
                ...
                <>
                  {category.image && (
                    <div className="mb-6 aspect-[3/1] w-full overflow-hidden rounded-2xl bg-muted">
                      <img
                        src={category.image}
                  ...
                  <SectionHeading
                    ...
                    title={category.name}
                    description={category.description || `Browse our full range of ${category.name} products.`}
                    align="left"
                  />
