// Sample / placeholder product catalog for the Products page demo.
// Prices, stock levels, ratings, and review counts are fabricated for
// demonstration purposes only — swap this file for a real feed/API
// before going live. Product photography reuses ITPlace's existing
// service photos as category art (see /public/services).
  ...
  CATEGORIES.forEach((category) => {
    ...
    templates.forEach((template, i) => {
      ...
      products.push({
        ...
        compareAtPrice: hasDiscount ? Math.round(template.price * 1.2 * 100) / 100 : null,
        rating,
        reviewCount,
        stock,
        inStock: stock > 0,
...
export const ALL_TAGS = [...new Set(PRODUCTS.flatMap((p) => p.tags))].sort();

export const ALL_COLORS = [...new Set(PRODUCTS.flatMap((p) => p.colors))].sort();
